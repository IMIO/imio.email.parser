# source: https://github.com/andrewferrier/email2pdf/blob/master/email2pdf
# last commit: 9a7dcce
...
HEADER_MAPPING = {"Author": "From", "Title": "Subject", "X-email2pdf-To": "To"}
...
def main(argv, syslog_handler, syserr_handler):
    ...
    if args.attachments:
        number_of_attachments = handle_attachments(
            input_email,
            output_directory,
    ...
    if (not args.body) and number_of_attachments == 0:
        logger.info(
            "First try: didn't print body (on request) or extract any attachments. Retrying with filenamed parts."
        )
        ...
        if number_of_attachments == 0:
            logger.warning(
                "Second try: didn't print body (on request) and still didn't find any attachments even when looked for "
                "referenced ones with a filename. Giving up."
            )

    if warning_count_filter.warning_pending:
        with open(
            get_modified_output_file_name(output_file_name, "_original.eml"), "w"
        ) as original_copy_file:
...
def handle_args(argv):
    ...
    parser = ArgumentParser(
        description="Converts emails to PDFs. "
    ...
    body_attachment_options.add_argument(
        "--no-body",
        dest="body",
        action="store_false",
        default=True,
    ...
    parser.add_argument(
        "--headers",
        action="store_true",
        help="Add basic email headers ("
        + ", ".join(FORMATTED_HEADERS_TO_INCLUDE)
...
def get_output_file_name(args, output_directory):
    if args.output_file:
        ...
        output_file_name = get_unique_version(
            os.path.join(
                output_directory, datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + ".pdf"
            )
        )
...
def set_up_warning_logger(logger, output_file_name):
    warning_logger_name = get_modified_output_file_name(
        output_file_name, "_warnings_and_errors.txt"
    )
...
def get_modified_output_file_name(output_file_name, append):
    ...
    partial_name = os.path.join(
        os.path.dirname(partial_name), os.path.basename(partial_name) + append
    )
    return partial_name
...
def handle_html_message_body(input_email, part):
    ...
    payload = re.sub(
        r"cid:([\w_@.-]+)", functools.partial(cid_replace, cid_parts_used), payload_str
    )
...
def output_body_pdf(input_email, payload, output_file_name):
    ...
    for error_pattern in WKHTMLTOPDF_ERRORS_IGNORE:
        (stripped_error, number_of_subs_made) = re.subn(
            error_pattern, "", stripped_error
        )
        if number_of_subs_made > 0:
            logger.debug(
                "Made "
                + str(number_of_subs_made)
                + " subs with pattern "
                + error_pattern
            )
    ...
    if wkh2p_process.returncode > 0 and original_error == "":
        ...
        raise FatalException(
            "wkhtmltopdf exited with rc = 0 but produced unknown stripped error output "
            + stripped_error
        )
...
def add_update_pdf_metadata(filename, update_dictionary):
    ...
    with open(filename, "rb") as input_file:
        ...
        for key in full_update_dictionary:
            ...
            info_dict.update(
                {NameObject(key): createStringObject(full_update_dictionary[key])}
            )
...
def extract_part_filename(part):
    ...
    if filename is not None:
        ...
        if decode_header(filename)[0][1] is not None:
            ...
            filename = str(
                decode_header(filename)[0][0], (decode_header(filename)[0][1])
            )
...
def get_type_extension(content_type):
    filetypes = (
        set(mimetypes.guess_all_extensions(content_type))
        - AUTOCALCULATED_FILENAME_EXTENSION_BLACKLIST
    )
...
def get_formatted_header_info(input_email):
    ...
    for header in FORMATTED_HEADERS_TO_INCLUDE:
        if input_email[header]:
            ...
            header_info = (
                header_info + "<b>" + header + "</b>: " + decoded_string + "<br/>"
            )
...
def call_main(argv, syslog_handler, syserr_handler):
    ...
    try:
        (warning_pending, mostly_hide_warnings) = main(
            argv, syslog_handler, syserr_handler
        )
...
if __name__ == "__main__":
    ...
    if _platform == "linux" or _platform == "linux2":
        ...
        logger_setup.warning(
            "I don't know this platform (" + _platform + "); cannot log to syslog."
        )
        SYSLOG_ADDRESS = None
    ...
    if SYSLOG_ADDRESS and os.path.exists(SYSLOG_ADDRESS):
        ...
        SYSLOG_FORMATTER = logging.Formatter(
            "%(pathname)s[%(process)d] %(levelname)s %(lineno)d %(message)s"
        )
